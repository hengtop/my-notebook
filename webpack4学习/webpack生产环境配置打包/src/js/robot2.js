...
const app = new Vue({
  ...
  data: {
    ...
    avg: 0,
    origin: [0, 0],//原点坐标
    coordinate: [0, 0],//机器人点坐标
    mapImg: {//图片实际尺寸和渲染尺寸,还有缩放
      naturalWidth: 0,
      naturalHeight: 0,
      width: 0,
      height: 0,
  ...
  methods: {
    ...
    //处理图片缩放
    handleImg() {
      this.$refs.maps.onload = function () {
        //app 为vue实例
        app.mapImg.height = this.height;
        app.mapImg.widthScale = parseFloat(this.width / this.naturalWidth);
        app.mapImg.heightScale = parseFloat(this.height / this.naturalHeight);
        app.handlePoint();
        //TODO 根据屏幕尺寸改变来获取图片渲染大小
      };
    },
    //处理点定位
    handlePoint(){
      ...
      let top = +this.origin[1] + this.coordinate[1];
      let left = +this.origin[0] + this.coordinate[0];
      top = this.mapImg.height - parseFloat(top * this.mapImg.heightScale);
      left = parseFloat(left * this.mapImg.widthScale);
      //获取边框值
      let imgWrapperPadding = +document.defaultView.getComputedStyle(imgWrapper,null)['padding'].split('px')[0];
      let widthPadding = point.clientWidth;
      let heightPadding = point.clientHeight;
      //设置点的位置,减去自身宽高一半
      point.style.top = top+imgWrapperPadding-(widthPadding/2) + 'px';
      point.style.left = left+imgWrapperPadding-(heightPadding/2) + "px";
    },
    //获取地址定位
    getLData() {
      axios({
        ...
        method: 'post',
        url: this.urlHead + '/robot/robot/getLData/',
      }).then(res => {
        const {data: {data}} = res
        this.coordinate[0] = +data[0];
        this.coordinate[1] = +data[1];
        this.handlePoint()
      })
    },
    //获取能谱仪数据
    ...
    //视频初始化
    videoInit() {
      this.player = new JSMpeg.Player('ws://192.168.1.5:8921', {
